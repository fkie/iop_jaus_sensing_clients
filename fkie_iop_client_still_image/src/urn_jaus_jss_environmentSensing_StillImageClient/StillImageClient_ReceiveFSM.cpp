...
void StillImageClient_ReceiveFSM::handleReportStillImageDataExtAction(ReportStillImageDataExt msg, Receive::Body::ReceiveRec transportData)
{
    /// Insert User Code HERE
    /// Insert User Code HERE
    JausAddress sender = transportData.getAddress();
    RCLCPP_DEBUG(logger, "received still image data ext from %s", sender.str().c_str());
    for (unsigned int i = 0; i < msg.getBody()->getStillImageDataList()->getNumberOfElements(); i++) {
        ReportStillImageDataExt::Body::StillImageDataList::StillImageDataRec* rec = msg.getBody()->getStillImageDataList()->getElement(i);
        unsigned short int id = rec->getSensorID();
        if (!(rec->getImageFrame()->getFormat() == 0 || rec->getImageFrame()->getFormat() == 2)) {
            RCLCPP_WARN(logger, "received unsupported still image format '%s', id: %d from %s",
                get_image_format(rec->getImageFrame()->getFormat()).c_str(), id, sender.str().c_str());
...
void StillImageClient_ReceiveFSM::p_connect_to_service(unsigned int id)
{
    if (has_remote_addr()) {
        ...
        drec.setCoordinateSystemEnum(1); // we use vehicle coordinate system
